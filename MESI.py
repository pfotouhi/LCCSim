...
def flush_buffer(buffer, PID):
	while buffer[PID]:
		protocol_stats["MESI"]["data_tran"] += 1 #DRAM access - Writeback
		protocol_stats["MESI"]["DRAM accesses"] += 1
		protocol_stats["MESI"]["Latency"] += L_DRAM
		count["FLUSH_RAM"] += 1
...
for line in fid:
	...
	#Initialization
	if "MESI" not in protocol_stats:
		protocol_stats["MESI"] = {}
		protocol_stats["MESI"]["recv_ctrl"] = 0
		protocol_stats["MESI"]["send_ctrl"] = 0
		protocol_stats["MESI"]["data_tran"] = 0
		protocol_stats["MESI"]["DRAM accesses"] = 0
		protocol_stats["MESI"]["Cache to cache"] = 0
		protocol_stats["MESI"]["Latency"] = 0
	
	if "MESI" not in protocol_stats_snoopy:
		protocol_stats_snoopy["MESI"] = {}
		protocol_stats_snoopy["MESI"]["recv_ctrl"] = 0
		protocol_stats_snoopy["MESI"]["send_ctrl"] = 0
		protocol_stats_snoopy["MESI"]["data_tran"] = 0
		protocol_stats_snoopy["MESI"]["DRAM accesses"] = 0
		protocol_stats_snoopy["MESI"]["Cache to cache"] = 0
		protocol_stats_snoopy["MESI"]["Latency"] = 0
	...
	if op == "RD":
		...
		# try:
			if (directory[var][PID] == "M"):
				protocol_stats["MESI"]["Latency"] += L_HIT
				count["RD_I_HIT"] += 1
				pass #Hit
				...
				if(is_shared):
					directory[var][PID] = "S"
					protocol_stats["MESI"]["data_tran"] += 1 #Cache to Cache (Not necessarily!)
					protocol_stats["MESI"]["DRAM accesses"] += 1
					protocol_stats["MESI"]["Latency"] += L_DRAM
					count["RD_I_RAM"] += 1
					pass #Miss
					...
					directory[var][PID] = "E"
					protocol_stats["MESI"]["data_tran"] += 1 #DRAM access - Read
					protocol_stats["MESI"]["DRAM accesses"] += 1
					protocol_stats["MESI"]["Latency"] += L_DRAM
					count["RD_I_RAM"] += 1
					pass #Miss
		...
		#Behaving as an atomic operation
...
protocol_stats_snoopy["MESI"]["send_ctrl"] = protocol_stats["MESI"]["send_ctrl"]
protocol_stats_snoopy["MESI"]["recv_ctrl"] = protocol_stats["MESI"]["send_ctrl"]*(int(num_cores)-1)
protocol_stats_snoopy["MESI"]["data_tran"] = protocol_stats["MESI"]["data_tran"]
protocol_stats_snoopy["MESI"]["DRAM accesses"] = protocol_stats["MESI"]["DRAM accesses"]
protocol_stats_snoopy["MESI"]["Cache to cache"] = protocol_stats["MESI"]["Cache to cache"]
protocol_stats_snoopy["MESI"]["Latency"] = protocol_stats["MESI"]["Latency"]
...
#Writing the results
output="MESI,"
for i in sorted(protocol_stats["MESI"]):
	output+=str(protocol_stats["MESI"][i]) + ","
output+=str(protocol_stats_snoopy["MESI"]["recv_ctrl"]) + ","
output+=str(protocol_stats_snoopy["MESI"]["send_ctrl"]) + ","
if DETAILED_MODE :
