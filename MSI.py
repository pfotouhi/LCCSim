...
def flush_buffer(buffer, PID):
	while buffer[PID]:
		protocol_stats["MSI"]["data_tran"] += 1 #DRAM access - Writeback
		protocol_stats["MSI"]["DRAM accesses"] += 1
		protocol_stats["MSI"]["Latency"] += L_DRAM
		count["FLUSH_RAM"] += 1
...
protocol_stats = {} #Per cache protocol, Directory based
...
for line in fid:
	...
	#Initialization
	if "MSI" not in protocol_stats:
		protocol_stats["MSI"] = {}
		protocol_stats["MSI"]["recv_ctrl"] = 0
		protocol_stats["MSI"]["send_ctrl"] = 0
		protocol_stats["MSI"]["data_tran"] = 0
		protocol_stats["MSI"]["DRAM accesses"] = 0
		protocol_stats["MSI"]["Cache to cache"] = 0
		protocol_stats["MSI"]["Latency"] = 0
	
	if "MSI" not in protocol_stats_snoopy:
		protocol_stats_snoopy["MSI"] = {}
		protocol_stats_snoopy["MSI"]["recv_ctrl"] = 0
		protocol_stats_snoopy["MSI"]["send_ctrl"] = 0
		protocol_stats_snoopy["MSI"]["data_tran"] = 0
		protocol_stats_snoopy["MSI"]["DRAM accesses"] = 0
		protocol_stats_snoopy["MSI"]["Cache to cache"] = 0
		protocol_stats_snoopy["MSI"]["Latency"] = 0
	...
	if op == "RD":
		...
		# try:
			if (directory[var][PID] == "M"):
				protocol_stats["MSI"]["Latency"] += L_HIT
				count["RD_I_HIT"] += 1
				pass #Hit
				...
				for proc in directory[var]:
					if (proc != PID and directory[var][proc] != "I"):
						if (directory[var][proc] == "M"):
							...
							raise NameError("Invalid State")
				directory[var][PID] = "S"
				protocol_stats["MSI"]["data_tran"] += 1 #DRAM access - Read
				protocol_stats["MSI"]["DRAM accesses"] += 1
				protocol_stats["MSI"]["Latency"] += L_DRAM
				count["RD_I_RAM"] += 1
				pass #Miss
...
protocol_stats_snoopy["MSI"]["send_ctrl"] = protocol_stats["MSI"]["send_ctrl"]
protocol_stats_snoopy["MSI"]["recv_ctrl"] = protocol_stats["MSI"]["send_ctrl"]*(int(num_cores)-1)
protocol_stats_snoopy["MSI"]["data_tran"] = protocol_stats["MSI"]["data_tran"]
protocol_stats_snoopy["MSI"]["DRAM accesses"] = protocol_stats["MSI"]["DRAM accesses"]
...
#Writing the results
output="MSI,"
for i in sorted(protocol_stats["MSI"]):
	output+=str(protocol_stats["MSI"][i]) + ","
output+=str(protocol_stats_snoopy["MSI"]["recv_ctrl"]) + ","
output+=str(protocol_stats_snoopy["MSI"]["send_ctrl"]) + ","
if DETAILED_MODE :
