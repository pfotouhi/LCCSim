...
def flush_buffer(buffer, PID):
	while buffer[PID]:
		protocol_stats["MOESI"]["data_tran"] += 1 #DRAM access - Writeback
		protocol_stats["MOESI"]["DRAM accesses"] += 1
		protocol_stats["MOESI"]["Latency"] += L_DRAM
		count["FLUSH_RAM"] += 1
...
for line in fid:
	...
	#Initialization
	if "MOESI" not in protocol_stats:
		protocol_stats["MOESI"] = {}
		protocol_stats["MOESI"]["recv_ctrl"] = 0
		protocol_stats["MOESI"]["send_ctrl"] = 0
		protocol_stats["MOESI"]["data_tran"] = 0
		protocol_stats["MOESI"]["DRAM accesses"] = 0
		protocol_stats["MOESI"]["Cache to cache"] = 0
		protocol_stats["MOESI"]["Latency"] = 0
	
	if "MOESI" not in protocol_stats_snoopy:
		protocol_stats_snoopy["MOESI"] = {}
		protocol_stats_snoopy["MOESI"]["recv_ctrl"] = 0
		protocol_stats_snoopy["MOESI"]["send_ctrl"] = 0
		protocol_stats_snoopy["MOESI"]["data_tran"] = 0
		protocol_stats_snoopy["MOESI"]["DRAM accesses"] = 0
		protocol_stats_snoopy["MOESI"]["Cache to cache"] = 0
		protocol_stats_snoopy["MOESI"]["Latency"] = 0
	...
	if var in caches[PID]:
		...
		if len(caches[PID]) < 256:
			...
			if directory[evict_loc][PID] == "M": #Adding to write buffer
				...
				pass
			elif directory[evict_loc][PID] == "O": #Adding to write buffer
				add_to_buffer(evict_loc, PID)
				pass
	...
	if op == "RD":
		...
		# try:
			if (directory[var][PID] == "M"):
				protocol_stats["MOESI"]["Latency"] += L_HIT
				count["RD_I_HIT"] += 1
				pass #Hit
			elif (directory[var][PID] == "O"):
				protocol_stats["MOESI"]["Latency"] += L_HIT
				count["RD_I_HIT"] += 1
				pass #Hit
					...
					if (proc != PID and directory[var][proc] != "I"):
						if (directory[var][proc] == "M"):
							...
							if var in write_buffer[proc]:
								...
								is_shared = True
								is_owned = True
								directory[var][proc] = "O"
								pass #M-->O
							pass #Contorl Message
						elif (directory[var][proc] == "O"):
							protocol_stats["MOESI"]["recv_ctrl"] += 1
							if var in write_buffer[proc]:
								flush_buffer(write_buffer, proc)
								pass #Flush
								...
								is_shared = True
								is_owned = True
								for other_proc in directory[var]:
									if (other_proc != PID and other_proc != proc and directory[var][other_proc] != "I"):
										protocol_stats["MOESI"]["data_tran"] += 1 #Cache to Cache
										protocol_stats["MOESI"]["Cache to cache"] += 1
								pass #Broadcast value
							pass #Contorl Message
				...
				if(is_shared):
					directory[var][PID] = "S"
					protocol_stats["MOESI"]["data_tran"] += 1
					if(is_owned):
						protocol_stats["MOESI"]["Cache to cache"] += 1 #Cache to Cache
						protocol_stats["MOESI"]["Latency"] += L_C2C
						count["RD_I_C2C"] += 1
					else:
						protocol_stats["MOESI"]["DRAM accesses"] += 1
						protocol_stats["MOESI"]["Latency"] += L_DRAM
					...
					directory[var][PID] = "E"
					protocol_stats["MOESI"]["data_tran"] += 1 #DRAM access - Read
					protocol_stats["MOESI"]["DRAM accesses"] += 1
					protocol_stats["MOESI"]["Latency"] += L_DRAM
					count["RD_I_RAM"] += 1
					pass #Miss
		...
		# try:
			if (directory[var][PID] == "M"):
					...
					if (proc != PID and directory[var][proc] != "I"):
						if (directory[var][proc] == "M"):
							...
							if var in write_buffer[proc]:
								...
								is_owned = True
								directory[var][proc] = "I"
								pass #Invalidating others, No need to Write back since we do C2C
							pass #Contorl Message
						elif (directory[var][proc] == "E"):
							directory[var][proc] = "I"
							protocol_stats["MOESI"]["recv_ctrl"] += 1
				...
				if(is_owned):
					...
					protocol_stats["MOESI"]["data_tran"] += 1
					protocol_stats["MOESI"]["DRAM accesses"] += 1 #DRAM access - Read
					protocol_stats["MOESI"]["Latency"] += L_DRAM
					count["WR_I_RAM"] += 1
					pass #Miss.
			else:
...
protocol_stats_snoopy["MOESI"]["send_ctrl"] = protocol_stats["MOESI"]["send_ctrl"]
protocol_stats_snoopy["MOESI"]["recv_ctrl"] = protocol_stats["MOESI"]["send_ctrl"]*(int(num_cores)-1)
protocol_stats_snoopy["MOESI"]["data_tran"] = protocol_stats["MOESI"]["data_tran"]
protocol_stats_snoopy["MOESI"]["DRAM accesses"] = protocol_stats["MOESI"]["DRAM accesses"]
protocol_stats_snoopy["MOESI"]["Cache to cache"] = protocol_stats["MOESI"]["Cache to cache"]
protocol_stats_snoopy["MOESI"]["Latency"] = protocol_stats["MOESI"]["Latency"]
...
#Writing the results
output="MOESI,"
for i in sorted(protocol_stats["MOESI"]):
	output+=str(protocol_stats["MOESI"][i]) + ","
output+=str(protocol_stats_snoopy["MOESI"]["recv_ctrl"]) + ","
output+=str(protocol_stats_snoopy["MOESI"]["send_ctrl"]) + ","
if DETAILED_MODE :
